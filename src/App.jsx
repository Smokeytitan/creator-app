...
import { getCreators, bulkImportCreators } from './services/creatorsService';
import { getCampaigns } from './services/campaignsService';

// Google Sheets CSV export URL (disabled for isolated version)
const GOOGLE_SHEET_URL = null;

export default function App() {
  const [activeTab, setActiveTab] = useState(() => {
    const stored = localStorage.getItem('activeTab');
    // Default to roster tab
    return stored || 'roster';
  });
  const [loading, setLoading] = useState(true);
  const [creators, setCreators] = useState([]);
  const [requests, setRequests] = useState([]);
  const [useSupabase, setUseSupabase] = useState(false); // Always use localStorage in isolated version
  ...
  useEffect(() => {
    ...
    const loadFromLocalStorage = async () => {
      // Use the localStorage service instead of direct localStorage access
      const [loadedCreators, loadedRequests] = await Promise.all([
        getCreators(),
        getCampaigns()
      ]);
      ...
      setCreators(loadedCreators);
      setRequests(loadedRequests);
      console.log('[App] ✓ Loaded from localStorage service');
    };
  ...
  // Data is automatically saved by the localStorage service when modified
  // No need to save here since all mutations go through the service layer

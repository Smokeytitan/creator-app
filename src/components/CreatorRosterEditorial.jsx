...
export default function CreatorRosterEditorial({ creators, setCreators, requests = [] }) {
            ...
            {editingId === c.id ? (
              ...
              <div className="flex flex-col h-full">
                ...
                {(c.posts || []).length > 0 && (() => {
                  ...
                  // Group posts by campaign (posts with same campaignId are part of same campaign)
                  const campaignGroups = new Map();
                  const standalonePosts = [];

                  posts.forEach(post => {
                    if (post.campaignId) {
                      if (!campaignGroups.has(post.campaignId)) {
                        campaignGroups.set(post.campaignId, []);
                      }
                      campaignGroups.get(post.campaignId).push(post);
                    } else {
                      standalonePosts.push(post);
                    }
                  });

                  // Calculate total impressions and cost across all campaigns
                  let totalImpressions = 0;
                  let totalCost = 0;
                  const numCampaigns = campaignGroups.size + standalonePosts.length;

                  // For campaign groups: count creator's costPerPost once per campaign, sum impressions
                  campaignGroups.forEach((campaignPosts) => {
                    // Sum impressions from all posts in this campaign
                    ...
                    // Count cost once per campaign (not per post)
                    if (c.costPerPost) {
                  ...
                  // For standalone posts: count each separately
                  ...
                  const avgImpressions = numCampaigns > 0 ? Math.round(totalImpressions / numCampaigns) : 0;
                  const cpm = totalImpressions > 0 ? (totalCost / totalImpressions) * 1000 : 0;
                  const avgCostPerCampaign = numCampaigns > 0 ? totalCost / numCampaigns : 0;
                        ...
                        {totalCost > 0 && (
                          <>
                            ...
                            {avgCostPerCampaign > 0 && (
                              <div className="bg-[var(--color-bg-tertiary)] rounded-lg p-3 border border-[var(--color-border)]">
                                <div className="text-xs text-[var(--color-text-tertiary)] mb-1 uppercase tracking-wide">Avg Cost/Campaign</div>
                                <div className="text-lg font-bold text-[var(--color-accent-secondary)] text-mono">
                                  ${avgCostPerCampaign.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </div>
                              </div>
                            )}
                          </>
                  ...
                  <div className="mt-4 space-y-3" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                      {(c.posts || []).length === 0 && addingPostId !== c.id ? (
                        ...
                        return (
                          <>
                            {/* Campaign-grouped posts */}
                            {Array.from(campaignGroups.entries()).map(([campaignId, campaignPosts]) => {
                              const campaign = requests.find(r => r.id === campaignId);
                              const totalImpressions = campaignPosts.reduce((sum, post) => {
                                if (post.impressions) {
                                  const imp = parseFloat(post.impressions.replace(/[^0-9.-]+/g, ''));
                                  return sum + (isNaN(imp) ? 0 : imp);
                                }
                                return sum;
                              }, 0);

                              const costValue = c.costPerPost ? parseFloat(c.costPerPost.replace(/[^0-9.-]+/g, '')) : 0;
                              const cpm = totalImpressions > 0 && costValue > 0 ? (costValue / totalImpressions) * 1000 : 0;

                              return (
                                <div key={`campaign-${campaignId}`} className="border-2 border-[var(--color-accent-primary)]/30 rounded-lg p-3 bg-[var(--color-accent-primary)]/5">
                                  {/* Campaign header */}
                                  <div className="mb-3 pb-2 border-b border-[var(--color-accent-primary)]/20">
                                    <div className="flex items-center justify-between mb-2">
                                      <h5 className="font-bold text-[var(--color-accent-primary)] text-sm">
                                    ...
                                    <div className="flex flex-wrap gap-3 text-xs text-[var(--color-text-secondary)]">
                                      {costValue > 0 && (
                                        <span className="inline-flex items-center text-mono">
                                          ...
                                          ${costValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </span>
                                      )}
                                      {totalImpressions > 0 && (
                                        <span className="inline-flex items-center text-mono">
                                      ...
                                      {cpm > 0 && (
                                        <span className="inline-flex items-center text-mono font-medium text-[var(--color-accent-primary)]">
                                          CPM: ${cpm.toFixed(2)}
                                        </span>
                                      )}
                                    </div>
                          ...
                          <div key={post.id} className="bg-[var(--color-bg-tertiary)] rounded-lg p-3 text-sm border border-[var(--color-border)]">
                            ...
                            {/* Standalone posts (not part of any campaign) */}
